...
import shutil
import pandas as pd
from myproject.models import Project, SubProject, Devices
...
def subprojectselected():
    if request.cookies.get('subproject_id'):
        return True
    else:
        return False
...
def folderpath():
    if subprojectselected():
        ...
        folder_path=os.path.join(basedir, 'directory',secure_filename(Project.query.get(project_id).name),secure_filename(SubProject.query.get(subproject_id).name))
        return folder_path
    else:
        return None
...
def delete():
    if request.cookies.get('project_id'):
        ...
        resp.delete_cookie("subproject_id")
...
def deletefolder():
    if subprojectselected():
        ...
        folder=SubProject.query.get(subproject_id)
...
def createfolder():
    ...
    if form.validate_on_submit():
        ...
        folder=SubProject(name,description,project_id)
        db.session.add(folder)
        all=SubProject.query.all()
        print(all)
        db.session.commit()
...
def viewfolder(id):
    ...
    resp.set_cookie('subproject_id', id)
    return resp
...
def dashboard():
    if subprojectselected():
        ...
        subproject=SubProject.query.get(subprojectid)
        ...
        for dev in all:
            ...
            if dev.pingable=="No":
                ...
                nossh+=1
        return render_template('dashboard.html',devices=all, name=subproject.name, description=subproject.description,total=total,noping=noping,nossh=nossh)
