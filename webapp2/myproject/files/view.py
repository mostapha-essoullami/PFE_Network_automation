...
def createconf(template_file,csv_parameter_file):
    ...
    if autoadddevice:
        ...
        for parameter in config_parameters:
            ...
            if ipexist:
                ip=parameter["ip"]
            dev=Devices(hostname,ip,parameter["username"],parameter["password"],parameter["devicetype"],folder_id)
            db.session.add(dev)
...
def listconfs():
    if subprojectselected():
        ...
        print(files)
        return render_template('confFiles.html',files=files)
    else:
        return redirect(url_for("projects.viewid"))  

@files_blueprint.route('/deleteconf/<file>')
def deleteconf(file):
    if subprojectselected():
        ...
        os.remove(os.path.join(tftp_path,file))
